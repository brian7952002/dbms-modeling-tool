...
const entity = (id: string, name: string, x = 100, y = 100): BaseNode & { weak: boolean } => ({
  id,
  kind: 'entity',
  name,
  x,
...
const seed = (nodes: BaseNode[] = [entity('e1', 'STUDENT')]): Diagram => ({
  nodes,
  edges: [],
});
...
describe('DiagramDoc convergence', () => {
  ...
  it('converges when three people edit at once', () => {
    ...
    expect((node as { weak?: boolean }).weak).toBe(true);
  });
});
