...
export function fromFile(raw: unknown): { diagram: Diagram; title: string; model: string } {
  ...
  return {
    ...
    // Files written before the ecosystem existed are all EER diagrams.
    model: typeof f.model === 'string' && f.model ? f.model : 'eer',
  };
}
