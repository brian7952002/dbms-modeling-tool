...
import type { BaseEdge, BaseNode, Diagram, Id, Point } from './types';
...
interface Props {
  /** The model that owns this diagram: its shapes, rules and styling. */
  model: ModelTool;
  diagram: Diagram;
  selection: Id[];
  ...
  onAddNodeAt: (kind: string, p: Point) => void;
...
export function Canvas({
  ...
  const bows = useMemo(() => {
    const groups = new Map<string, BaseEdge[]>();
  ...
  // Whatever the shapes need beyond a single element — marker rotation, which
  // connectors are doubled — is the model's business, computed once here.
  const decorations = useMemo(() => model.decorate(diagram), [model, diagram]);
  ...
  const beginConnect = useCallback(
    (id: Id) => {
      ...
      if (a && b && model.inferEdge(a, b, diagram)) {
    ...
    [connectFrom, diagram, dispatch, nodeById, model],
  );

  const onNodePointerDown = (e: React.PointerEvent, node: BaseNode) => {
    e.stopPropagation();
  ...
  return (
    <svg
      ...
      onDrop={(e) => {
        e.preventDefault();
        const kind = e.dataTransfer.getData('application/x-eer-node');
        if (!kind) return;
        const p = toDiagram(e.clientX, e.clientY);
        onAddNodeAt(kind, { x: snapValue(p.x), y: snapValue(p.y) });
      }}
    >
      <style>{model.css(theme)}</style>
      <defs>
          ...
          {diagram.edges.map((e) => {
            ...
            return (
              <model.EdgeShape
                ...
                decoration={decorations.edges.get(e.id)}
        ...
        <g className="nodes">
          {diagram.nodes.map((n) => (
            <model.NodeShape
              ...
              decoration={decorations.nodes.get(n.id)}
