...
import type { Diagram, DiagramNode, Edge, Id } from '../model/types';
...
export class DiagramDoc {
  ...
  snapshot(): Diagram {
    ...
    for (const entry of this.nodes.values()) {
      nodes.push(entry.toJSON() as DiagramNode);
    }
    const edges: Edge[] = [];
    for (const entry of this.edges.values()) {
      edges.push(entry.toJSON() as Edge);
    }
  ...
  insert(nodes: DiagramNode[], edges: Edge[]) {
    this.transact(() => {
  ...
  updateNode(id: Id, patch: Partial<DiagramNode>) {
  ...
  updateEdge(id: Id, patch: Partial<Edge>) {
