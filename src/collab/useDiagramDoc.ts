...
import type { Diagram, Id, Point } from '../model/types';
import { createEdge, createNode, inferEdge } from '../model/factory';
 ...
 */
export function useDiagramDoc(initial: { diagram: Diagram; title: string }): DocBinding {
  ...
  const dispatch = useCallback<React.Dispatch<Action>>(
    (action) => {
      switch (action.type) {
        ...
        case 'addNode': {
          const node = createNode(action.kind, action.x, action.y);
          doc.addNode(node);
        ...
        case 'updateNode': {
          ...
          if (patch.name !== undefined) {
            const existing = doc.nodes.get(action.id);
            const nodeKind = existing?.get('kind') as Diagram['nodes'][number]['kind'] | undefined;
            // Marker shapes are sized by their symbol, not their label.
            if (nodeKind && nodeKind !== 'isa' && nodeKind !== 'union') {
              Object.assign(patch, fitSize(nodeKind, patch.name));
            }
          }
        ...
        case 'connect': {
          ...
          if (!a || !b) return;
          const spec = inferEdge(a, b, snapshot);
          if (!spec) return;
